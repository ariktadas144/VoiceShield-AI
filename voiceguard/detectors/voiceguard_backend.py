"""VoiceGuard backend -- the adapted RawNet2 detector this project trained.

A thin wrapper. It does not change how VoiceGuard works: the same `audio_utils` front
...
class VoiceGuardDetector:
    ...
    def predict(self, audio_16k_mono: np.ndarray) -> DetectionResult:
        ...
        if str(REPO_ROOT) not in sys.path:
            sys.path.insert(0, str(REPO_ROOT))
        from audio_utils import pad, peak_normalise
        ...
        # VoiceGuard's own windowing: non-overlapping nb_samp windows, tail tiled.
        window = self.nb_samp
