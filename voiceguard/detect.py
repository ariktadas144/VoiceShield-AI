"""Score one audio file with a selectable detector backend.
    ...
    python detect.py --backend voiceguard --audio sample.wav
    python detect.py --backend dhwani     --audio sample.wav

Selection happens here, at runtime. Neither backend knows the other exists.

`eval.py` remains the VoiceGuard-specific tool with its original CLI intact; this is the
...
def main() -> int:
    ...
    p.add_argument("--checkpoint", default=None, help="voiceguard: checkpoint path")
    ...
    kwargs = {}
    if args.backend == "voiceguard" and args.checkpoint:
