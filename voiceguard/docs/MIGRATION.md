# Migration: voiceguard working directory → VoiceShield-AI

Everything from the standalone `voiceguard/` working directory now lives on this branch.
...
| inference backends | `detectors/` — VoiceGuard and the selectable Dhwani backend |
...
are tracked here instead. `voiceguard/.gitignore` negates the ignore rule for those five
