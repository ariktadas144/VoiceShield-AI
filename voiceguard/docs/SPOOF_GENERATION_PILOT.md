...
either generator is safe to train VoiceGuard on. **No VoiceGuard training was started and
...
VoiceGuard's spoof half currently contains xtts_v2 and freevc24 only. Both are
...
## 9. The comparison that decides it — against data VoiceGuard already trains on

The §7–8 contrasts use the pilot's reference corpus as bonafide, and that corpus is not
what VoiceGuard trains on. Scored against the actual training bonafide, with the spoofs
...
## 10. Low-pass probe through VoiceGuard v1
...
Training on it would hand VoiceGuard a "count the high-frequency energy" route to ~0.73
...
7. **No training was run**, so nothing here shows what VoiceGuard would actually *learn*
...
`numpy<=1.26.4` and would have downgraded the detector's `numpy==2.1.3`. **The VoiceGuard
