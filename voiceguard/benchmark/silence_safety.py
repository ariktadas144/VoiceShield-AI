...
def run_backend(args) -> int:
    ...
    Same conditions, same clips, same speech-ratio gate as the VoiceGuard path -- only
...
def run(args) -> int:
    ...
    if args.backend and args.backend != "voiceguard":
...
def main() -> int:
    ...
    p.add_argument("--backend", default="voiceguard",
                   help="detector backend: voiceguard (default) or dhwani")
