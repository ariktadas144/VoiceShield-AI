...
later:
  ...
  * both generators receive the identical reference set, so speaker cannot predict
    generator either.
  * the Hindi references come from SherryT997 train, which VoiceGuard v1 trained on.
...
def main() -> int:
    ...
    print(f"  references not in the indic manifest at all : {untraced:4d}   "
          f"(OpenSLR -- never part of any VoiceGuard split)")
    ...
    for d in args.external:
        ...
        # it is the same clip VoiceGuard already learned from. A reference sitting in a
    ...
    print(f"  Hindi references inside VoiceGuard v1 training bonafide: {hindi_train}")
