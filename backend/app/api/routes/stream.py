...
async def websocket_analyze_stream(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            if msg.get("text") is not None:
                ...
                if ctl.get("type") == "eof":
                    ...
                    if buf.size >= SAMPLE_RATE // 2:
                        ...
                        await _emit(websocket, pad, window_seq, sample_offset, partial=True)
            ...
            while buf.size >= win:
                await _emit(websocket, buf[:win], window_seq, sample_offset)
                window_seq += 1
                buf = buf[HOP_SAMPLES:]
...
async def _emit(ws: WebSocket, window: np.ndarray, seq: int, offset: int, partial: bool = False):
