"""Fine-tune VoiceShield's RawNet on Indic speech.
...
class Dataset_Indic(Dataset):
    """Reads a VoiceShield-format manifest: one `<path> <label>` per line."""

    def __init__(self, manifest, root=None, nb_samp=64_600, sample_rate=16_000,
                 normalise=True, rawboost=0, trim=False):
        self.manifest = Path(manifest)
        # Manifest paths are relative to the package root, not to the manifest's own
        # directory. They used to be absolute paths into one machine's home directory,
        # which meant no other checkout could read them; package-relative makes the
        # manifests portable and is what every consumer now assumes.
        self.root = Path(root) if root else PKG_ROOT
...
def main() -> int:
    ...
    if args.init == "pretrained":
        ...
        # Continue from an earlier VoiceShield-Indic model rather than the official
