...
def main() -> int:
    ...
    ap.add_argument("--model_save_path", default="checkpoints_asdg")
    ...
    for ep in range(args.num_epochs):
        ...
        for bi, (x, y, d) in enumerate(tl):
            ...
            real = (y == 0) & (d >= 0)
            if real.any():
                zr = GradReverse.apply(z[real], lam)
                logits = domain_head(zr)
                l_ada = F.cross_entropy(logits, d[real])
                agg["dom_ok"] += (logits.argmax(1) == d[real]).sum().item()
                agg["dom_n"] += int(real.sum())
            else:
                l_ada = z.new_zeros(())
        ...
        row = {"epoch": ep + 1, "bce": agg["bce"] / agg["n"], "ada": agg["ada"] / agg["n"],
               ...
               "dev_eer": eer, "domain_acc": dom_acc, "grl_lambda": lam,
        ...
        print(f"epoch {ep+1}/{args.num_epochs}  bce={row['bce']:.4f} "
              ...
              f"| dev_EER={100*eer:.2f}%  domain_acc={dom_acc:.1f}% "
              f"(unif 33.3 / majority {maj:.1f})  lambda={lam:.3f}", flush=True)
        ...
        if eer < best_eer:
            ...
            torch.save({"state_dict": model.state_dict(), "config": cfg, "dev_eer": eer,
                        ...
                        "init_ckpt": args.init_ckpt, "asdg": True,
